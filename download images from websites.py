...
for id in range(523):
    response = requests.get(f'http://www.sexinsex.net/bbs/forum-252-{id}.html')
    print(f"we are at page {id}")
    # Parse the HTML content of the page
    soup = BeautifulSoup(response.content, 'html.parser')

    # Find all the images on the page
    tags = soup.find_all('th', class_='lock')
    ...
    for tag in tags:
        ...
        for image in images:
            try:
                ...
                open(f"D:/Media/nosql/photo/other/{image_url.split('/')[-1]}", 'wb').write(response.content)
